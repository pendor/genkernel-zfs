...
new_create_initramfs() {
	local tmprd="${TMPDIR}/initramfs-${KV}" common_opts='-f -H' opts=''
	...
	then
		...
		then
			print_info 1 "initramfs: Not copying kernel modules and firmware..."
			opts+=\ --no-kernel
		else
			isTrue "${FIRMWARE}" && opts+=" --fwdir ${FIRMWARE_DIR}"
			# TODO: FIRMWARE_FILES
			# TODO: Use Dracut option -I
			# TODO: -I, --install LIST
			# TODO:     install the space separated list of files into the
			# TODO:     initramfs.

		fi

		isTrue "${DISKLABEL}" && print_info 'Skipping explicit install of blkid.  Should be installed automatically if needed.'
	...
	print_info "dracut ${common_opts} ${tmprd} ${KV}"
	dracut ${common_opts} ${opts}  "${tmprd}" "${KV}"
